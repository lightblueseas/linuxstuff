...
# KeePass Installation Script for Multiple OS
# This script installs KeePass2 based on the detected OS
...
declare -A descriptions=(
    ["keepass2"]="KeePass2 (Cross-platform password manager)."
)
...
case "$OS" in
    manjaro|arch)
        descriptions=(
            ["keepass"]="KeePass (Cross-platform password manager)."
        )
        ;;
    ...
    macos)
        descriptions=(
            ["keepassxc"]="KeePassXC (Cross-platform password manager fork)."
        )
        ;;
esac
...
if [[ ${#TO_INSTALL[@]} -eq 0 ]]; then
    info "All KeePass components are already installed. Exiting."
    exit 0
fi
...
case "$OS" in
    ...
    linux)
        if command -v snap &> /dev/null; then
            ...
            error "Snap is not installed. Please install Snap or use your package manager to install KeePass."
            exit 1
        fi
        ;;
...
info "Verifying KeePass installation..."
for component in "${TO_INSTALL[@]}"; do
...
info "KeePass installation completed successfully on $OS."
